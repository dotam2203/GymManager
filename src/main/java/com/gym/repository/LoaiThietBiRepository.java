...
@Repository
public interface LoaiThietBiRepository  extends CrudRepository<LoaiThietBi, String> {
	@Query("SELECT c FROM LoaiThietBi c WHERE c.maLoai = :maLoai")
	public LoaiThietBi findByMaLoai(@Param("maLoai") String maLoai);
	...
	@Query("FROM LoaiThietBi c ORDER BY c.maLoai DESC")
